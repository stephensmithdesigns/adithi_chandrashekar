...
function pixflow_str_get_html($str, $lowercase=true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
	$dom = new pixflow_simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
...
// dump html dom tree
function pixflow_dump_html_tree($node, $show_attr=true, $deep=0)
{
	$node->dump($node);
}
 ...
 * @package PlaceLocalInclude
 */
class pixflow_simple_html_dom_node
{
	...
	function dump($show_attr=true, $deep=0)
	{
		...
		echo esc_attr($lead).esc_html($this->tag);
...
class pixflow_simple_html_dom
{
	...
	protected function prepare($str, $lowercase=true, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
	{
		...
		$this->root = new pixflow_simple_html_dom_node($this);
		$this->root->tag = 'root';
	...
	protected function parse()
	{
		...
		// text
		$node = new pixflow_simple_html_dom_node($this);
		++$this->cursor;
	...
	function createElement($name, $value=null) {return @pixflow_str_get_html("<$name>$value</$name>")->first_child();}
	function createTextNode($value) {return @end(pixflow_str_get_html($value)->nodes);}
	function getElementById($id) {return $this->find("#$id", 0);}
	function getElementsById($id, $idx=null) {return $this->find("#$id", $idx);}
	function getElementByTagName($name) {return $this->find($name, 0);}
