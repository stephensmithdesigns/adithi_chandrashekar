...
class pixflow_Instagram {
    ...
    public function getLoginUrl($scope = array('basic')) {
        ...
        echo("Error: getLoginUrl() - The parameter isn't an array or invalid scope permissions used.");
    }
    ...
    public function modifyRelationship($action, $user) {
        ...
        echo("Error: modifyRelationship() | This method requires an action command and the target user id.");
    }
    ...
    public function pagination($obj, $limit = 0) {
        ...
        echo("Error: pagination() | This method doesn't support pagination.");
    }
    ...
    protected function _makeCall($function, $auth = false, $params = null, $method = 'GET') {
        ...
        $request =   wp_remote_get($apiCall);
        // Get the body of the response
        $jsonData = wp_remote_retrieve_body( $request );

        if (!$jsonData) {
            echo("Error: _makeCall() - cURL error: " );
        }
    ...
    private function _makeOAuthCall($apiData) {
        ...
        $request =   wp_remote_post($apiHost,$args);
        // Get the body of the response
        $jsonData = wp_remote_retrieve_body( $request );
        if (!$jsonData) {
            echo("Error: _makeOAuthCall() - cURL error: ");
        }
