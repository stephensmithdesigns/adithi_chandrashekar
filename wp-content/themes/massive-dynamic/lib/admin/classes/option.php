...
class VP_Option
{
	...
	public function __construct(array $configs)
	{
		...
		$configs = array_merge(array(
			...
			'layout'                => 'fixed',
			'page_title'            => esc_attr__( 'Vafpress Options', 'massive-dynamic' ),
			'menu_label'            => esc_attr__( 'Vafpress Options', 'massive-dynamic' ),
			'priority'              => 10,
		), $configs);
		...
		else throw new Exception(esc_attr__( 'Option Key is required', 'massive-dynamic' ), 1);
		if(isset($template)) $this->set_template($template);
		else throw new Exception(esc_attr__( 'Template Array/File is required', 'massive-dynamic' ), 1);
		if(isset($page_slug)) $this->set_page_slug($page_slug);
		else throw new Exception(esc_attr__( 'Page Slug is required', 'massive-dynamic' ), 1);
	...
	{
		VP_WP_Util::admin_notice(esc_attr__("Development Mode is Active, options' values won't be saved into database.", 'massive-dynamic'), false);
	}
	...
	function ajax_import_option()
	{
		...
		if($result['status'])
		{
			...
			{
				$result['status']  = false;
				$result['message'] = esc_attr__("Can not be empty", 'massive-dynamic');
			}
			else
			{
				...
				{
					$result['status']  = false;
					$result['message'] = esc_attr__("Invalid data", 'massive-dynamic');
				}
			}
		}
	...
	function ajax_export_option()
	{
		...
		if($result['status'])
		{
			...
			$result = array(
				'status' => true,
				'message'=> esc_attr__("Successful", 'massive-dynamic'),
				'option' => $sr_options,
			);
		}
	...
	function vp_verify_nonce()
	{
		...
		{
			$result['status']  = true;
			$result['message'] = esc_attr__("Successful", 'massive-dynamic');
		}
		else
		{
			$result['status']  = false;
			$result['message'] = esc_attr__("Unverified Access", 'massive-dynamic');
		}
		return $result;
	}
	...
	public function init_options_set()
	{
		...
		if( is_string($this->get_template()) and is_file($this->get_template()) )
			...
			throw new Exception(esc_attr__( 'Invalid template supplied', 'massive-dynamic' ), 1);
		...
		if( $this->use_util_menu() )
		{
			...
			$util_menu->set_title(esc_attr__('Utility', 'massive-dynamic'));
			...
			$restore_section->set_title(esc_attr__('Restore Default', 'massive-dynamic'));
			...
			$exim_section->set_title(esc_attr__('Export/Import', 'massive-dynamic'));
