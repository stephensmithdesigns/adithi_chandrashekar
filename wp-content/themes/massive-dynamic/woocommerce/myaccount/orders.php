...
<article class="account-title">
	<h1><?php esc_attr_e('Orders','massive-dynamic'); ?></h1>
</article>
		...
		<table class="woocommerce-orders-table woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_orders account-orders-table">
			...
			<tbody>
				<?php foreach ( $customer_orders->orders as $customer_order ) :
                    ...
                    <?php foreach ( wc_get_account_orders_columns() as $column_id => $column_name ) : ?>
						<td class="woocommerce-orders-table__cell-<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
								<?php if ( has_action( 'woocommerce_my_account_my_orders_column_' . $column_id ) ) : ?>
									<?php do_action( 'woocommerce_my_account_my_orders_column_' . $column_id, $order ); ?>

								<?php elseif ( 'order-number' === $column_id ) : ?>
									<a href="<?php echo esc_url( $order->get_view_order_url() ); ?>">
										<?php echo _x( '#', 'hash before order number', 'massive-dynamic' ) . $order->get_order_number(); ?>
									</a>
								...
								<?php elseif ( 'order-total' === $column_id ) : ?>
									...
									printf( _n( '%1$s for %2$s item', '%1$s for %2$s items', $item_count, 'massive-dynamic' ), $order->get_formatted_order_total(), $item_count );
									?>
    ...
    <?php else : ?>
		<div class="woocommerce-Message woocommerce-Message--info woocommerce-info">
			<a class="woocommerce-Button button" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
				<?php _e( 'Go Shop', 'massive-dynamic' ) ?>
			</a>
			<?php _e( 'No order has been made yet.', 'massive-dynamic' ); ?>
		</div>
	<?php endif; ?>
