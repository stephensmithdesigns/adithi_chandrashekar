...
function snp_builder_create_element($type, $step_index, $el_index, $args)
{
    ...
    if (isset($args['action']) && $args['action']!='') {
        if ($args['action']=='gotostep') {
            ...
            if (isset($args['open_type']) && $args['open_type'] == 'blank') {
                $data .= ' onclick="snp_open_link(this, true);"';
            } else {
                $data .= ' onclick="snp_open_link(this, false);"';
            }
    ...
    if ($type == 'text') {
        ...
        $return .= '<div class="g-recaptcha" data-sitekey="' . snp_get_option('recaptcha_api_key') . '"></div>';
        $return .= '</div>';
    } else if ($type == 'button') {
        $tabindex = '';
